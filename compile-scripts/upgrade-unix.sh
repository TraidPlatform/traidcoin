...
SOURCES_PATH="$SOURCES_PATH_DEFAULT/traidcoin"
BACKUP_PATH_DEFAULT=$HOME/traidbackup
BACKUP_PATH=$BACKUP_PATH_DEFAULT
REPO="https://github.com/traidcoin/traidcoin"
BRANCH="master"
SWAP_FILE="$HOME/traidcoin-swap"
...
if [ -d "../.git" ]; then
	...
	if [ -f "$SOURCES_PATH/bin/traidd" ]; then
		rm -f $SOURCES_PATH/bin/traidd
		echo -e "${BLUE}Removed old binary in $SOURCES_PATH/bin/${NC}"
	fi
	if [ -f "../bin/traidd" ]; then
      rm ../bin/traidd
	...
	SOURCES_PATH="${SOURCES_PATH:-$SOURCES_PATH_DEFAULT}/traidcoin"
...
sudo killall -9 traidd
if [ -f "/usr/local/bin/traidd" ]; then
  sudo rm -f /usr/local/bin/traidd
  echo -e "${BLUE}Removed traidd in /usr/local/bin/ - To be replaced with new version${NC}"
fi
...
if [ ! -f $SOURCES_PATH/bin/traidd ]; then
	echo -e "\n${RED}ERROR: traidd binary could not be created${NC}"
	exit -2
fi
...
sudo cp $SOURCES_PATH/bin/traidd /usr/local/bin/
sudo strip /usr/local/bin/traidd

# Backup data dir
if [ -d "$HOME/.traid/" ]; then
	echo -e "\n${GREEN}Backing up ~/traid (including wallet.dat)${NC}"
...
# Done
echo -e "\n${GREEN}Upgrade Complete - You can now run traidd or fill out the config file in ~/traid${NC}"
