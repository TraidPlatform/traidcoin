...
{
    resize(855, 650);
    setWindowTitle(tr("Traid") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("Traid");
    setStyleSheet("#Traid { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Traid address"));
    ...
    aboutAction = new QAction(QIcon(":icons/traid"), tr("&About Traid"), this);
    aboutAction->setToolTip(tr("Show information about Traid"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Traid"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #ffffff; font-size: 16px; font-family: 'Roboto', Arial,Times,Times New Roman,serif; font-weight: 350; margin: 5px 0; padding: 5px 15px; border: none;}"
                           "QToolButton:hover { color: #ffffff; background-color: #515954; border: none;}"
                           "QToolButton:checked { color: #ffffff; background-color: #5d6660; border: none;}"
                           "QToolButton:pressed { color: #ffffff; background-color: #5d6660; border: none;}"
                           "#tabs { color: #ffffff; background-color: #4a524d; border: none; margin-top:0px; margin-bottom:0;}");

    QLabel* header = new QLabel();
    header->setMinimumSize(142, 142);
    header->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    header->setPixmap(QPixmap(":/images/header"));
    header->setMaximumSize(205,205);
    ...
    foreach(QAction *action, toolbar->actions()) {
        toolbar->widgetForAction(action)->setFixedWidth(205);
    }
}
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Traid client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Traid client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Traid network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Traid address or malformed URI parameters."));
    }
