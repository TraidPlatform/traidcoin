...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTraidAmount"))
        settings.setValue("nAnonymizeTraidAmount", 1000);
    nAnonymizeTraidAmount = settings.value("nAnonymizeTraidAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTraidAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeTraidAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTraidAmount:
            return QVariant(nAnonymizeTraidAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTraidAmount:
            nAnonymizeTraidAmount = value.toInt();
            settings.setValue("nAnonymizeTraidAmount", nAnonymizeTraidAmount);
            emit AnonymizeTraidAmountChanged(nAnonymizeTraidAmount);
            break;
        default:
            break;
        }
