...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (int64_t)GetProofOfStakeReward(pindexBest->nHeight, 0, 0, GetTime());
}
...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest->nHeight, nCoinAge, 0, GetTime());
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue", (int64_t)GetProofOfStakeReward(pindexBest->nHeight, 0, 0, GetTime()))); //pprev
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Traid is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Traid is downloading blocks...");
...
Value getwork(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Traid is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Traid is downloading blocks...");
