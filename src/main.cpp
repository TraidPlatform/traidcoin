...
unsigned int nStakeMinAge = 3 * 60 * 60; // 3h
unsigned int nModifierInterval = 8 * 60; //8 min time to elapse before new modifier is computed
...
const string strMessageMagic = "Traid Signed Message:\n";
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees, int64_t nTimee)
{
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(pindex->nHeight, nCoinAge, nFees, nTime);
