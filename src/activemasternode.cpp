...
//
// Bootup the masternode, look for a 60000 TRAID input and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if (status != MASTERNODE_REMOTELY_ENABLED)
        {
            ...
            if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
                ...
                notCapableReason = "TRAID Masternode successfully started";
                ...
                /* rewards are not supported in Traid.conf */
...
{
    CTraidcoinAddress address(collateralAddress);
